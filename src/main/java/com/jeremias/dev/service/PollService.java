...
import com.jeremias.dev.exception.BadRequestException;
import com.jeremias.dev.exception.ResourceNotFoundException;
import com.jeremias.dev.mapper.DefaultMapper;
import com.jeremias.dev.models.Choice;
import com.jeremias.dev.models.ChoiceVoteCount;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(UserPrincipal currentUser, int page, int size) {
        ...
        // Retrieve Polls // set at the things 
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        
        Page<Poll> polls = pollRepository.findAll(pageable);

        if(polls.getNumberOfElements() == 0) {
        	//if is empty we return object with empty values
        ...
        List<PollResponse> pollResponses = polls. map(poll -> {
            return  DefaultMapper.mapPollToPollResponse(poll,
                    choiceVoteCountMap,
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username, UserPrincipal currentUser, int page, int size) {
        validatePageNumberAndSize(page, size);
        //we find the user orThrow an exception

        User user = userRepository.findByUsername(username)
                .orElseThrow(() -> new ResourceNotFoundException("User", "username", username));

        // Retrieve all polls created by the given username
        									//page //size //properties //field
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        
        Page<Poll> polls = pollRepository.findByCreatedBy(user.getId(), pageable);
        //we verify polls isn't empty
        if (polls.getNumberOfElements() == 0) {
            return new PagedResponse<>(Collections.emptyList(), polls.getNumber(),
                    polls.getSize(), polls.getTotalElements(), polls.getTotalPages(), polls.isLast());
        }
    ...
    public PollResponse getPollById(Long pollId, UserPrincipal currentUser) {
        ...
        return DefaultMapper.mapPollToPollResponse(poll, choiceVotesMap,
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, UserPrincipal currentUser) {
        ...
        return DefaultMapper.mapPollToPollResponse(poll, choiceVotesMap, creator, vote.getChoice().getId());
    }
    ...
    private Map<Long, Long> getChoiceVoteCountMap(List<Long> pollIds) {
    	...
    	//we can use choiceVoteCount be a class or a record
